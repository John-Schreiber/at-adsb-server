...
- **Full stack (readsb built from source)**: If you don't already have readsb running, [`docker-compose.full.yml`](docker-compose.full.yml) builds [wiedehopf/readsb](https://github.com/wiedehopf/readsb) from source alongside this daemon, with a Caddy sidecar serving readsb's JSON output and an optional Cloudflare Tunnel service for exposing the WebSocket stream publicly. See the file's header comment for prerequisites.
