...
for arg in "$@"; do
  if [ "$arg" = "generate-stream-key" ]; then
    printf 'Stream signing key generated.\n'
    printf 'Public key (did:key): zQ3shtestkey\n'
    printf 'Private key (hex): deadbeefcafef00d\n'
    printf '\n'
    printf 'Add to your environment:\n'
    printf '  STREAM_SIGNING_KEY_HEX=deadbeefcafef00d\n'
    exit 0
  fi
done
...
DOCKER
  ...
  # Remove args file so each case starts fresh (only created when docker runs)
  rm -f "$FAKE_DOCKER_ARGS"
}
...
test_successful_setup_with_signing() {
  make_case successful
  # Input: PDS, handle, password, name, readsb, batch(=default), poll(=default),
  #   stats(=default), stream-endpoint(skip), consent(y), lat, lon, signing(y)
  run_wizard $'https://pds.example.test\noperator.example\nsecret!$\nHome receiver\nhttp://readsb.example.test:8080\n\n\n\n\ny\n38.8977\n-77.0365\ny' "$CASE_DIR/elsewhere"
  ...
  assert_file_contains "$CASE_DIR/.env" 'STREAM_SIGNING_KEY_HEX="deadbeefcafef00d"'
  args=$(args_text)
  ...
  printf '%s\n' "$args" | grep -Fx -- 'generate-stream-key' >/dev/null || fail 'generate-stream-key not called'
  assert_file_not_contains "$CASE_DIR/output" 'secret!$'
  assert_no_temp_env
}

test_stream_endpoint_passed_to_register() {
  make_case stream-endpoint
  local ep='wss://relay.example.com/xrpc/at.adsb.broadcast.subscribeEvents'
  local input
  input=$(printf 'https://pds.example\nhandle\npassword\nName\nhttp://readsb\n\n\n\n%s\ny\n12.34\n56.78\nn' "$ep")
  run_wizard "$input" "$CASE_DIR"
  assert_equal "$RUN_STATUS" "0"
  assert_file_contains "$CASE_DIR/.env" "STREAM_ENDPOINT=\"${ep}\""
  args=$(args_text)
  printf '%s\n' "$args" | grep -Fx -- '--stream-endpoint' >/dev/null || fail 'missing --stream-endpoint flag'
  printf '%s\n' "$args" | grep -Fx -- "$ep" >/dev/null || fail 'stream endpoint value not passed to register'
}

test_signing_skipped() {
  make_case signing-skip
  run_wizard $'https://pds.example\nhandle\npassword\nName\nhttp://readsb\n\n\n\n\ny\n1\n2\nn' "$CASE_DIR"
  assert_equal "$RUN_STATUS" "0"
  assert_file_not_contains "$CASE_DIR/.env" 'STREAM_SIGNING_KEY_HEX'
  args=$(args_text)
  if printf '%s\n' "$args" | grep -Fx -- 'generate-stream-key' >/dev/null; then
    fail 'generate-stream-key called when user declined'
  fi
  assert_file_contains "$CASE_DIR/output" 'docker compose run --rm daemon generate-stream-key'
}
...
test_defaults_applied() {
  ...
  assert_file_contains "$CASE_DIR/.env" 'QUEUE_DB_PATH="/data/at-adsb-queue.db"'
  assert_file_contains "$CASE_DIR/.env" 'POLL_INTERVAL_S="5"'
  assert_file_not_contains "$CASE_DIR/.env" 'STREAM_ENDPOINT'
  assert_file_not_contains "$CASE_DIR/.env" 'STREAM_SIGNING_KEY_HEX'
}
...
test_punctuation_round_trip() {
  ...
  input=$(printf '%s\n' "$service" "$handle" "$password" "$name" "$readsb" "" "" "" "" y 12.3 -45.6 n)
  run_wizard "$input" "$CASE_DIR"
  assert_equal "$RUN_STATUS" "0"
...
test_newline_inputs_rejected() {
  ...
  local field
  for field in service handle password name readsb batch_window poll_interval stats_interval stream_endpoint consent latitude longitude signing; do
